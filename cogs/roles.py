...
class Roles(commands.Cog):
    ...
    async def addexistingroles(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if role.name in allowed:
            await interaction.response.send_message(f"Role '{role.name}' is already in the assignable list.", ephemeral=True)
            return
        
        # Check if the bot has permission to manage this role
        if role.position >= interaction.guild.me.top_role.position:
            await interaction.response.send_message(f"Cannot add role '{role.name}' - it is higher than or equal to my highest role.", ephemeral=True)
            return
