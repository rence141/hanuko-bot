...
class Game(commands.Cog):
    ...
    async def profile(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        if equipped_pet:
            ...
            if equipped_pet in user_data.get("pets", []):
                if pet_obj:
                    ...
                    pet_display = equipped_pet
            else:
                pet_display = f"Legacy/Missing Pet: {equipped_pet}"
                pet_color = 0x95a5a6  # Neutral gray
        # else: pet_display remains 'None' and color is default
        embed = discord.Embed(
    ...
    async def daily(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if last_daily:
                try:
                    ...
                    if (now - last_dt).total_seconds() < 86400:
                        ...
                        left = timedelta(seconds=86400 - (now - last_dt).total_seconds())
                        embed = discord.Embed(
            ...
            if not cooldown_active:
                ...
                update_user(interaction.user.id, credits=user_data["credits"], last_daily=user_data["last_daily"])
                embed = discord.Embed(
                    description=f"✅ {interaction.user.mention}, you claimed your daily reward: 100 credits!",
                    color=discord.Color.green()
                )
                await interaction.response.send_message(embed=embed)
    ...
    async def weekly(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if last_weekly:
                try:
                    ...
                    if (now - last_dt).total_seconds() < 604800:
                        cooldown_active = True
                        left = timedelta(seconds=604800 - (now - last_dt).total_seconds())
                        embed = discord.Embed(
            ...
            if not cooldown_active:
                ...
                update_user(interaction.user.id, credits=user_data["credits"], last_weekly=user_data["last_weekly"])
                embed = discord.Embed(
                    description=f"✅ {interaction.user.mention}, you claimed your weekly reward: 500 credits!",
                    color=discord.Color.green()
                )
                await interaction.response.send_message(embed=embed)
    ...
    async def recontainscp(self, interaction: discord.Interaction):
        ...
        user_hp = 100
        user_atk = 10
        # Find equipped weapon (first weapon in inventory not damaged)
        equipped_weapon = None
        for item in inventory:
            if item in WEAPON_BONUS and item not in damaged_items:
                equipped_weapon = item
                break
        if equipped_weapon:
            user_atk += WEAPON_BONUS[equipped_weapon]
        # Add suit bonus
        if "Containment Suit" in inventory and "Containment Suit" not in damaged_items:
            user_hp += SUIT_BONUS
        # Battle log
        log = [f"You encounter **{scp['name']}**! {scp['desc']}",
               f"Your HP: {user_hp} | ATK: {user_atk}",
    ...
    async def gunsmith(self, interaction: discord.Interaction, item: str):
        ...
        embed = discord.Embed(description=f"🛠️ You repaired **{item}** for {cost} credits!", color=discord.Color.green())
        await interaction.response.send_message(embed=embed)
